 ...
 *          the size of the hashmap is defined in INIT_HASH_SIZE constant, defaults to 32.
 *
 ...
 * @details By an algorithm it calculates the position of the node in the hashmap on the key string value.
 *          DO NOT USE this function into source code!
 *
 * @param key (char*) The key for index calculation.
 * @return (unsigned int) The index calculated.
 */
 ...
 * @details Search the node to remove by the key string value and return a pointer to it without deallocating it.
 ...
 * @details Search the node by the key string value without removing it, just return a pointer to it.
 *
 * @param hash (Hash) The hashmap where search the node.
 * @param key (char*) The key string.
 * @return (Node*) A pointer to the node in the hashmap.
 */
 ...
 * @return (unsigned int) The number of records into the hashmap.
 */
